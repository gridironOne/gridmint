...
OUTPUT?=$(BUILDDIR)/gridmint

BUILD_TAGS?=gridmint

COMMIT_HASH := $(shell git rev-parse --short HEAD)
LD_FLAGS = -X github.com/gridironOne/gridmint/version.DymintGitCommitHash=$(COMMIT_HASH)
...
build:
	CGO_ENABLED=$(CGO_ENABLED) go build $(BUILD_FLAGS) -tags '$(BUILD_TAGS)' -o $(OUTPUT) ./cmd/gridmint
.PHONY: build

install:
	CGO_ENABLED=$(CGO_ENABLED) go install $(BUILD_FLAGS) -tags $(BUILD_TAGS) ./cmd/gridmint
.PHONY: install

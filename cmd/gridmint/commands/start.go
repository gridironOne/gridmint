...
import (
	...
	cfg "github.com/gridironOne/gridmint/config"
	"github.com/gridironOne/gridmint/conv"
	"github.com/gridironOne/gridmint/node"
	"github.com/gridironOne/gridmint/rpc"
...
// These are exposed for convenience of commands embedding a gridmint node
...
func startInProcess(config *cfg.NodeConfig, tmConfig *tmcfg.Config, logger log.Logger) error {
	...
	logger.Info("starting node with ABCI gridmint in-process")
	...
	server := rpc.NewServer(gridmintNode, tmConfig.RPC, logger)
	err = server.Start()
	if err != nil {
		return err
	}

	logger.Debug("initialization: gridmint node created")
	if err := gridmintNode.Start(); err != nil {
		return err
	}

	logger.Info("Started gridmint node")

	// Stop upon receiving SIGTERM or CTRL-C.
	tmos.TrapSignal(logger, func() {
		if gridmintNode.IsRunning() {
			if err := gridmintNode.Stop(); err != nil {

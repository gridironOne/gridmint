...
transaction load against gridmint and measuring their resulting latency.
...
Below is a basic invocation of the report tool with a data directory under `/home/test/.gridmint/data/`

```bash
./build/report --data-dir ~/.gridmint/data
```
...
```bash
./build/report --data-dir ~/.gridmint/data --csv out.csv
```
